...
st.markdown("""
    ...
    .stat-item { font-size: 18px !important; font-weight: 800 !important; }

    .stat-label { font-size: 14px; color: #555; margin-right: 8px; font-weight: 600; }
...
if buy_file and sell_file:
    ...
    try:
        ...
        df_base['매입행'] = df_base['원본행_x'].apply(lambda x: str(x) if x != 0 else "-")

        df_base['매출행'] = df_base['원본행_y'].apply(lambda x: str(x) if x != 0 else "-")

        

        # 1. 전체

        df_all = df_base[['매칭코드', '상품명', '매입행', '매출행', '매입수량', '수량', '수량오차', '합계금액', '합계', '금액오차']]
        ...
        df_analysis = df_all[['코드', '품명', '매입행', '매출행', '수량오차', '금액오차']]
        ...
        df_buy_unmoved = merged[mask_buy][['매칭코드', '상품명', '원본행_x', '일자_숫자_x', '매입수량', '합계금액']].copy()

        df_buy_unmoved.columns = ['코드', '품명', '매입행', '매입일자', '매입수량', '매입금액']
        ...
        df_sell_current = merged[mask_sell][['매칭코드', '품명', '원본행_y', '일자_숫자_y', '수량', '합계']].copy()

        df_sell_current.columns = ['코드', '품명', '매출행', '매출일자', '매출수량', '매출금액']
        ...
        if not target_df.empty:
            ...
            t_sum = target_df['금액오차'].sum()

            st.markdown(f'<div class="stat-container"><div class="stat-item"><span class="stat-label">📈 (+)합계</span><span style="color:#007BFF;">{p_sum:,.0f}원</span></div><div class="stat-item"><span class="stat-label">📉 (-)합계</span><span style="color:#FF4B4B;">{m_sum:,.0f}원</span></div><div class="stat-item"><span class="stat-label">⚖️ 총 오차</span>{t_sum:,.0f}원</div></div>', unsafe_allow_html=True)
        ...
        bc2.markdown(f'<div class="report-card"><p class="card-label">✅ 최종 매출 금액</p><p style="font-size:13px; color:#E74C3C;">설정 제외: -{abs(ex_sell_sum):,.0f}원</p><p class="price-text">{f_sell_total:,.0f}원</p></div>', unsafe_allow_html=True)

        bc3.markdown(f'<div class="report-card"><p class="card-label">⚖️ 결과 차액</p><p style="font-size:13px; color:#666;">옵션 반영 완료</p><p class="diff-text" style="color:{"#007BFF" if f_diff >=0 else "#FF4B4B"};">{f_diff:,.0f}원</p></div>', unsafe_allow_html=True)
