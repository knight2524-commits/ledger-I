...
if buy_file and sell_file:
    try:
        ...
        # 3. 미이월/당월 추출 로직 (불일치 건 + 날짜 조건)
        def check_date_match(date_list, target_dates):
            return any(d in target_dates for d in date_list) if target_dates else False

        # 매입 미이월: 매출에 없고 + 선택한 날짜에 해당
        unmoved_mask = ~b_grouped['매칭코드'].isin(s_grouped['상품코드'])
        df_buy_unmoved = b_grouped[unmoved_mask & b_grouped['일자_숫자'].apply(lambda x: check_date_match(x, buy_dates))].copy()
        
        # 매출 당월: 매입에 없고 + 선택한 날짜(1,2,3일 포함)에 해당
        current_mask = ~s_grouped['상품코드'].isin(b_grouped['매칭코드'])
        df_sell_current = s_grouped[current_mask & s_grouped['일자_숫자'].apply(lambda x: check_date_match(x, sell_dates))].copy()
        ...
        if not df_buy_unmoved.empty:
            df_b_unmoved_disp = df_buy_unmoved[['매칭코드', '상품명', '원본행', '매입수량', '합계금액']].copy()
            df_b_unmoved_disp.columns = ['코드', '품명', '매입행', '매입수량', '매입금액']
            df_b_unmoved_disp['금액오차'] = df_b_unmoved_disp['매입금액']
        else:
            df_b_unmoved_disp = pd.DataFrame(columns=['코드', '품명', '매입행', '매입수량', '매입금액', '금액오차'])

        # 당월 디스플레이 (1,2,3일 포함)
        if not df_sell_current.empty:
            df_s_curr_disp = df_sell_current[['상품코드', '품명', '원본행', '수량', '합계']].copy()
            df_s_curr_disp.columns = ['코드', '품명', '매출행', '매출수량', '매출금액']
            df_s_curr_disp['금액오차'] = -df_s_curr_disp['매출금액']
        else:
            df_s_curr_disp = pd.DataFrame(columns=['코드', '품명', '매출행', '매출수량', '매출금액', '금액오차'])
        ...
        display_map = {
            ...
            "매입처 미이월": df_b_unmoved_disp, 
            "매출처 당월": df_s_curr_disp
        }
        ...
        unmoved_ex_val = df_buy_unmoved['합계금액'].sum() if exclude_buy_amt else 0
        current_ex_val = df_sell_current['합계'].sum() if exclude_sell_amt else 0
        
        f_buy_total = df_b_proc['합계금액'].sum() - unmoved_ex_val
        f_sell_total = df_s_proc['합계'].sum() - current_ex_val
        f_diff = f_buy_total - f_sell_total

        c1, c2, c3 = st.columns(3)
        c1.markdown(f'<div class="report-card"><p class="card-label">최종 매입 합계</p><p style="color:red; font-size:12px;">미이월 제외: -{unmoved_ex_val:,.0f}</p><p class="price-text">{f_buy_total:,.0f}원</p></div>', unsafe_allow_html=True)
        c2.markdown(f'<div class="report-card"><p class="card-label">최종 매출 합계</p><p style="color:red; font-size:12px;">당월 제외: -{current_ex_val:,.0f}</p><p class="price-text">{f_sell_total:,.0f}원</p></div>', unsafe_allow_html=True)
        c3.markdown(f'<div class="report-card"><p class="card-label">최종 결과 차액</p><p style="font-size:12px;">&nbsp;</p><p class="diff-text" style="color:{"#007BFF" if f_diff>=0 else "#FF4B4B"};">{f_diff:,.0f}원</p></div>', unsafe_allow_html=True)
