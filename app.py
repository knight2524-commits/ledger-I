...
st.set_page_config(page_title="장부 오차 검증 툴 (공급가 기준)", layout="wide")
...
st.title("📊 매입/매출 원장 상세 비교 분석 (부가세 제외)")
...
def load_data(file):
    if file.name.endswith('.csv'):
        try: return pd.read_csv(file, encoding='cp949', dtype=str)
...
if buy_file and sell_file:
    try:
        ...
        # 2. 매칭 및 부가세 제외 처리 (1.1로 나누기)
        # 매입 전처리 (상품명 기준)
        df_buy_proc = df_buy.copy()
        df_buy_proc['매칭코드'] = df_buy_proc['상품명'].astype(str).str.strip().str.upper()
        df_buy_proc = df_buy_proc[df_buy_proc['매칭코드'] != 'NAN']
        df_buy_proc['매입수량'] = pd.to_numeric(df_buy_proc['매입수량'], errors='coerce').fillna(0)
        
        # 합계금액을 부가세 제외 공급가로 변환
        raw_buy_amt = pd.to_numeric(df_buy_proc['합계금액'], errors='coerce').fillna(0)
        df_buy_proc['합계금액'] = (raw_buy_amt / 1.1).round(0) 

        # 매출 전처리 (품번 기준)
        df_sell_proc = df_sell.copy()
        df_sell_proc['매칭코드'] = df_sell_proc['품번'].astype(str).str.strip().str.upper()
        df_sell_proc = df_sell_proc[df_sell_proc['매칭코드'] != 'NAN']
        df_sell_proc['수량'] = pd.to_numeric(df_sell_proc['수량'], errors='coerce').fillna(0)
        
        # 합계를 부가세 제외 공급가로 변환
        raw_sell_amt = pd.to_numeric(df_sell_proc['합계'], errors='coerce').fillna(0)
        df_sell_proc['합계'] = (raw_sell_amt / 1.1).round(0)
        ...
        st.download_button(
            label=f"📥 {view_option} 데이터 엑셀로 내보내기 (부가세 제외)",
            data=excel_data,
            file_name=f"검증결과_공급가_{view_option}_{datetime.now().strftime('%Y%m%d')}.xlsx",
        ...
        if not target_df.empty:
            ...
            st.markdown(f'<div class="stat-container"><div class="stat-item">📈 (+)합계(공급가): {p_sum:,.0f}원</div><div class="stat-item">📉 (-)합계(공급가): {m_sum:,.0f}원</div><div class="stat-item">⚖️ 총 오차(공급가): {target_df["금액오차"].sum():,.0f}원</div></div>', unsafe_allow_html=True)
        ...
        c1.markdown(f'<div class="report-card"><p class="card-label">최종 매입 공급가</p><p style="color:red; font-size:12px;">제외: -{ex_buy_sum:,.0f}</p><p class="price-text">{f_buy_total:,.0f}원</p></div>', unsafe_allow_html=True)
        c2.markdown(f'<div class="report-card"><p class="card-label">최종 매출 공급가</p><p style="color:red; font-size:12px;">제외: -{ex_sell_sum:,.0f}</p><p class="price-text">{f_sell_total:,.0f}원</p></div>', unsafe_allow_html=True)
        c3.markdown(f'<div class="report-card"><p class="card-label">결과 차액 (공급가)</p><p style="font-size:12px;">&nbsp;</p><p class="diff-text" style="color:{"#007BFF" if (f_buy_total-f_sell_total)>=0 else "#FF4B4B"};">{(f_buy_total-f_sell_total):,.0f}원</p></div>', unsafe_allow_html=True)
