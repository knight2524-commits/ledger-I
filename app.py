...
st.title("📊 매입/매출 원장 상세 비교 분석 툴 (공급가 기준)")
...
if buy_file and sell_file:
    try:
        ...
        # 공급가 계산 (1.1로 나누기)
        df_buy_proc['합계금액'] = (pd.to_numeric(df_buy_proc['합계금액'], errors='coerce').fillna(0) / 1.1).round(0)
        ...
        # 공급가 계산 (1.1로 나누기)
        df_sell_proc['합계'] = (pd.to_numeric(df_sell_proc['합계'], errors='coerce').fillna(0) / 1.1).round(0)
        ...
        st.download_button(
            label=f"📥 {view_option} 데이터 엑셀로 다운로드",
            data=excel_data,
        ...
        st.markdown('<p class="sub-title" style="font-weight:bold;">✅ 설정 반영 후 최종 금액 (공급가 기준)</p>', unsafe_allow_html=True)
